...
class OrderBookTest : public ::testing::Test
{
  protected:
    ...
    std::shared_ptr<Order> makeLimit(OrderId id, Side side, int instrument_id, double price, double qty = 1.0f)
    {
        return std::make_shared<Order>(id, Type::LIMIT, side, 1, price, qty, 1000);
    }

    std::shared_ptr<Candle> makeCandle(int instrument_id, double low, double high, uint64_t ts = 2000)
    {
        CandleData data{};
        data.timestamp = ts;
        data.low = low;
        ...
        return std::make_shared<Candle>(data, (int32_t)instrument_id);
    }
...
{
    int instrument_id = 1;
    book->onOrder(makeLimit(1, Side::BUY, instrument_id, 100.0f));

    book->onCandle(makeCandle(instrument_id, 99.0f, 105.0f));
...
{
    int instrument_id = 1;
    book->onOrder(makeLimit(10, Side::BUY, instrument_id, 100.0f));
    book->onCandle(makeCandle(instrument_id, 100.0f, 105.0f));
...
TEST_F(OrderBookTest, NoFillLogic)
{
    int instrument_id = 1;
    book->onOrder(makeLimit(1, Side::BUY, instrument_id, 100.0f));
    book->onCandle(makeCandle(instrument_id, 100.01f, 105.0f));
...
TEST_F(OrderBookTest, TimestampCheck)
{
    ...
    book->onCandle(makeCandle(1, 90.0f, 110.0f, candle_ts));
