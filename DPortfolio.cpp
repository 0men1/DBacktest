...
void DPortfolio::process_sell_order(Order &o)
{
    ...
    if (p.quantity > 0 && p.quantity >= o.quantity())
    {
        ...
        m_fNetLiquidity += o.price() * o.quantity() - (m_fCommission > 0 ? (m_fCommission * o.quantity()) : 0); // If commision is not 0 then include it
...
void DPortfolio::process_buy_order(Order &o)
{
    ...
    // If position quantity > 0 -> update the average price, increase quantity, decrease net liquid
    if (p.quantity > 0)
    {
    ...
    // If no position (e.g. quantity = 0) Assign average price, quantity, and decrease from net liquid
