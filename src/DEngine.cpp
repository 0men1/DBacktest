...
DEngine::DEngine(std::unique_ptr<IStrategy> strategy,
                 const std::string &filepath, uint32_t buffer_size) {

  m_context = std::make_shared<BacktestContext>(
      std::make_unique<DReader>(filepath, buffer_size), std::move(strategy));
